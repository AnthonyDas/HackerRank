...
// C:\Users\AnthonyDas\Documents\GitHub\HackerRank\HackerRank\Interview Preparation Kit\Greedy Algorithms\Reverse Shuffle Merge.cpp
...
std::string reverseShuffleMerge(const std::string &s) {
	...
	for (auto it = s.crbegin(); it != s.crend(); ++it) {
		...
		// If we have a lexicographically earlier char than A.back() and there are
		// still opportunities to replace the same A.back() char later on, then opt
		// for this by unwinding A.
